...
class Cart(object):
    ...
    def increase(self, key):
        ...
        if not cart_item:
            raise ValueError(
                f"Товар не в корзине")
        supply = cart_item['supply']
        if cart_item['quantityOrdered'] + 1 > supply:
            raise ValueError(
                f"Максимальное количество данного товара в корзине: {supply}")
        else:
            cart_item['quantityOrdered'] += 1
        self.save()
    ...
    def add(self, product_id, color_id, size_name, quantity=1):
        ...
        self.cart[key] = {
            'key': key,
            'productSlug': matching_products.product.slug,
            'colorSlug': matching_products.color.slug,
    ...
    def reduce(self, key):
        ...
        if cart_item:
            if cart_item['quantityOrdered'] == 1:
                raise ValueError(' == 1')
            cart_item['quantityOrdered'] -= 1
        else:
    ...
    def get_total_price(self):
        return sum(int(Decimal(item['price']) * item['quantityOrdered']) for item in self.cart.values())
